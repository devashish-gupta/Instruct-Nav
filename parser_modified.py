...
def create_samples(input_path, language_dict, num_instruction = 2, obs_window: int = 6, pred_window: int = 8) -> dict:
    ...
    for i in range(traj_len - seq_len):
        ...
        frame_instructions = [] # instructions for this window
        ...
        if len(frame_instructions) > 0:         
            instruction_list.append(frame_instructions)
    most_recent_instructions = [inst[-num_instruction:] for inst in instruction_list]
    ...
    post_processed = {
        ...
        "instructions": most_recent_instructions
    }
...
if __name__ == "__main__":
    ...
    if args.create_samples:
        ...
        list_pickles = list(parsed_path.parent.glob("**/*.pkl"))
        ...
        for file_name in bar:
            ...
            post_processed = create_samples(
                file_name, language_dict=language_dict, num_instruction=1, obs_window=cfg.obs_len, pred_window=cfg.pred_len
            )
            if bool(base_dict):
